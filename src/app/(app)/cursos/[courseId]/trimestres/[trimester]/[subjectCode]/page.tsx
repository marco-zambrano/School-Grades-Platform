...
export default async function SubjectPage({
  ...
  return (
    <>
      ...
      />
      {!hasEnoughAportes ? <p className="mb-5 rounded-2xl border border-amber-700/50 bg-amber-950/30 p-4 font-medium text-amber-200">Faltan actividades de aporte: se recomiendan al menos 9 entre individuales y grupales.</p> : null}
      <div className="app-card overflow-x-auto rounded-3xl">
        <table className="min-w-full border-collapse text-left text-sm">
          <thead className="app-card-muted">
            <tr>
              <th className="app-card-muted sticky left-0 z-10 min-w-52 p-3">Estudiante</th>
              {subject.activities.map((activity) => <th key={activity.id} className="min-w-28 p-3 text-center"><span className="block">{activity.name}</span><span className="text-muted mt-1 block text-xs font-normal">{activity.type.replaceAll("_", " ")}</span></th>)}
          ...
          <tbody>
            {course.students.map((student) => {
              ...
              return <tr key={student.id} className="border-t">
                <th className="sticky left-0 z-10 bg-[var(--surface)] p-3 font-semibold">{student.fullName}</th>
      ...
      {course.students.length === 0 ? <p className="text-muted mt-4">Añada estudiantes a la nómina antes de registrar notas.</p> : null}
      <section className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_26rem]">
        <div className="app-card rounded-3xl p-6">
          ...
          <ul className="mt-3 space-y-2">
            {subject.activities.map((activity) => <li key={activity.id} className="app-card-muted flex items-center justify-between gap-3 rounded-xl px-3 py-2"><span>{activity.name}{activity.locked ? <span className="text-muted ml-2 text-sm">(predeterminada)</span> : null}</span>{!activity.locked ? <DeleteActivityButton activityId={activity.id} /> : null}</li>)}
          </ul>
        </div>
